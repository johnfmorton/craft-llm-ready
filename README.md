...
* Auto-generates a `/llms.txt` site index following the [llms.txt specification](https://llmstxt.org/) — and it can be switched off independently if you only want the `.md` URLs. Its site description can be plain text in the settings or sourced from a field on a Single or global set so content editors own it.
